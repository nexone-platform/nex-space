...
# The API runs `prisma db push` at start-up. New tables and nullable columns are
# safe; anything else is not, and this copy is the only way back.
...
# Node's fetch and Prisma's own client do the asking, so nothing extra has to be
# installed in the images.
API_HEALTH='fetch("http://localhost:3001/health").then(r => r.json()).then(d => { if (!d.ok) { console.error(JSON.stringify(d)); process.exit(1); } }).catch(e => { console.error(String(e)); process.exit(1); })'
...
GAME_UP='fetch("http://localhost:2567/").then(r => { if (r.status >= 500) { console.error("status " + r.status); process.exit(1); } }).catch(e => { console.error(String(e)); process.exit(1); })'
...
GUEST_ROUTE='out=$(wget -S -O /dev/null http://localhost/guest-pass/__probe__ 2>&1 || true); case "$out" in *404*) exit 0 ;; *200*) echo "answered 200 - nginx served the SPA, the location block is missing"; exit 1 ;; *) echo "no usable answer: $(echo "$out" | tr -d "\n" | cut -c1-110)"; exit 1 ;; esac'
API_VIA_NGINX='wget -qO- http://localhost/health 2>/dev/null | grep -q ok || { echo "no JSON from /health through nginx"; exit 1; }'
WEB_SERVES='wget -qO- http://localhost/ 2>/dev/null | grep -q "<title>" || { echo "index.html did not come back"; exit 1; }'
